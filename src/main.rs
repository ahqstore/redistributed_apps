use ahqstore_types::{
  AHQStoreApplication, AppRepo, DownloadUrl, InstallType, InstallerFormat, InstallerOptions,
  InstallerOptionsWin32,
};
use base64::prelude::*;
...
Redistributed by AHQ Store (ahqsecret@gmail.com)"#
      ...
      icon: BASE64_STANDARD.encode(std::fs::read("./firefox.png").unwrap()),
    ...
    std::fs::write(
      "./firefox.json",
      to_string(to_string(&app).unwrap().as_bytes()).unwrap(),
    )
    .unwrap();
  }
}
